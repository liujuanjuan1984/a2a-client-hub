...
This project supports local developer convenience defaults, but production deployments must use hardened settings.

Set `APP_ENV=production` in `backend/.env` to enable strict startup validation. The backend will refuse to start when baseline checks fail.

## Backend Baseline (Enforced)

When `APP_ENV=production`, the backend enforces:

- `JWT_ALGORITHM` is restricted to asymmetric algorithms (`RS*`/`ES*`) with explicit `JWT_PRIVATE_KEY_PEM` and `JWT_PUBLIC_KEY_PEM`.
...
- `WS_ALLOWED_ORIGINS` is explicitly configured and does not contain `*` or localhost origins.
...
- If `AUTH_REFRESH_COOKIE_SAMESITE=none`, `AUTH_REFRESH_COOKIE_SECURE` must stay `true`.
...
- If HTTP exceptions are needed for local debugging, use development-only build configuration and never reuse it in production artifacts.
