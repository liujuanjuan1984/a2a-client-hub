...
def test_invoke_hub_agent_ws_success(monkeypatch, mock_user):
    ...
    monkeypatch.setattr(
        "app.api.routers.hub_a2a_agents.get_a2a_service", lambda: mock_service
    )
    ...
    monkeypatch.setattr(
        "app.api.routers.hub_a2a_agents.hub_a2a_runtime_builder.build", mock_build
    )
    monkeypatch.setattr("app.api.routers.hub_a2a_agents.validate_message", lambda x: [])
