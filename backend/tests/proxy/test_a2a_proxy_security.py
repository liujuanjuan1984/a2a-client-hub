...
from app.features.personal_agents import router as personal_agents_router
...
def test_proxy_requires_allowlist(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        personal_agents_router._normalize_card_url(
...
def test_proxy_allows_subdomain_suffix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        personal_agents_router._normalize_card_url("https://api.example.com/card")
...
def test_proxy_blocks_private_ip(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        personal_agents_router._normalize_card_url("http://127.0.0.1:8000/card")
