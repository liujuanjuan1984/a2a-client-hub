...
@pytest.mark.asyncio
async def test_hub_card_validate_accepts_session_query_result_envelope_by_method(
    async_session_maker, async_db_session, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(settings, "a2a_proxy_allowed_hosts", ["example.com"])
    ...
    agent_id, user = await _create_allowlisted_hub_agent(
        async_session_maker=async_session_maker,
        async_db_session=async_db_session,
        admin_email="admin_validate_envelope@example.com",
        user_email="alice_validate_envelope@example.com",
        token="secret-token-validate-envelope",
    )
