...
async def test_shared_invoke_keeps_runtime_validation_as_502(
    ...
    async def _raise_runtime_validation(*args, **kwargs):
        raise SharedAgentRuntimeValidationError("runtime validation failed")

    monkeypatch.setattr(
        hub_router.shared_agent_runtime_builder, "build", _raise_runtime_validation
    )
...
async def test_shared_invoke_returns_409_for_missing_user_credential(
    ...
    async def _raise_user_credential(*args, **kwargs):
        raise SharedAgentUserCredentialRequiredError("credential required")

    monkeypatch.setattr(
        hub_router.shared_agent_runtime_builder, "build", _raise_user_credential
    )
