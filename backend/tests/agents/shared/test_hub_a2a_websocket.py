...
def test_invoke_hub_agent_ws_success(monkeypatch, mock_user):
    ...
    monkeypatch.setattr(
        "app.features.agents.shared.router.get_a2a_service", lambda: mock_service
    )
    ...
    monkeypatch.setattr(
        "app.features.agents.shared.router.shared_agent_runtime_builder.build",
        mock_build,
    )
    monkeypatch.setattr(
        "app.features.agents.shared.router.validate_message", lambda _value: []
    )
