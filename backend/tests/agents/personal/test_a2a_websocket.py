...
from app.runtime.ws_ticket import WsTicketNotFoundError, ws_ticket_service
...
def test_invoke_agent_ws_success(monkeypatch, mock_user):
    ...
    monkeypatch.setattr(
        "app.features.agents.personal.router.get_a2a_service", lambda: mock_service
    )
    ...
    monkeypatch.setattr(
        "app.features.agents.personal.router.a2a_runtime_builder.build", mock_build
    )

    # Mock validate_message to return empty list
    monkeypatch.setattr(
        "app.features.agents.personal.router.validate_message", lambda _value: []
    )
...
def test_invoke_agent_ws_invalid_conversation_id_returns_error_event(
