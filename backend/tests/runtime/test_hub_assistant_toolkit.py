...
async def test_hub_assistant_toolkit_executes_first_wave_operations(
    async_db_session,
) -> None:
...
async def test_hub_assistant_toolkit_updates_agent_config(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    ...
    assert result.payload["agent"]["tags"] == ["hub-assistant", "toolkit"]


async def test_hub_assistant_toolkit_checks_agent_health(
    async_db_session,
...
async def test_hub_assistant_toolkit_rejects_invalid_schedule_inputs(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    agent = await create_a2a_agent(
    ...
    with pytest.raises(HubAssistantToolInputError) as exc_info:
...
async def test_hub_assistant_toolkit_sends_session_message(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
...
async def test_hub_assistant_toolkit_gets_latest_session_messages(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
...
async def test_hub_assistant_toolkit_reads_and_updates_follow_up_state(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
    ...
    monkeypatch.setattr(
        task_service_module.hub_assistant_task_service,
        "get_follow_up_state",
    ...
    assert get_result.payload["hub_assistant_conversation_id"] == str(
...
async def test_hub_assistant_toolkit_starts_agent_sessions(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
