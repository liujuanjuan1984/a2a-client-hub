...
from app.core.security import create_hub_assistant_access_token
...
from app.features.hub_assistant.shared.hub_assistant_mcp import (
    ...
    execute_hub_assistant_mcp_operation,
...
async def test_hub_assistant_mcp_server_lists_first_wave_tools() -> None:
    tools = await hub_assistant_mcp_server.list_tools()
    tool_names = {tool.name for tool in tools}

    assert "hub_assistant.agents.list" in tool_names
    assert "hub_assistant.agents.get" in tool_names
    assert "hub_assistant.jobs.list" in tool_names
    assert "hub_assistant.jobs.get" in tool_names
    assert "hub_assistant.followups.get" in tool_names
    assert "hub_assistant.followups.set_sessions" in tool_names
    assert "hub_assistant.sessions.list" in tool_names
    assert "hub_assistant.sessions.get" in tool_names
    assert "hub_assistant.sessions.get_latest_messages" in tool_names


async def test_hub_assistant_write_mcp_server_lists_write_tools() -> None:
    tools = await hub_assistant_write_mcp_server.list_tools()
    tool_names = {tool.name for tool in tools}

    assert "hub_assistant.agents.check_health" in tool_names
    assert "hub_assistant.agents.check_health_all" in tool_names
    assert "hub_assistant.agents.create" in tool_names
    assert "hub_assistant.agents.update_config" in tool_names
    assert "hub_assistant.agents.delete" in tool_names
    assert "hub_assistant.agents.start_sessions" in tool_names
    assert "hub_assistant.jobs.create" in tool_names
    assert "hub_assistant.jobs.pause" in tool_names
    assert "hub_assistant.jobs.resume" in tool_names
    assert "hub_assistant.jobs.update" in tool_names
    assert "hub_assistant.jobs.delete" in tool_names
    assert "hub_assistant.followups.get" in tool_names
    assert "hub_assistant.followups.set_sessions" in tool_names
    assert "hub_assistant.sessions.update" in tool_names
    assert "hub_assistant.sessions.archive" in tool_names
    assert "hub_assistant.sessions.send_message" in tool_names
    assert "hub_assistant.sessions.unarchive" in tool_names


async def test_execute_hub_assistant_mcp_operation_returns_swival_envelope(
    async_db_session,
) -> None:
    ...
    list_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
        operation_id="hub_assistant.jobs.list",
        arguments={"page": 1, "size": 20},
        db=async_db_session,
    )
    get_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
...
async def test_execute_hub_assistant_mcp_operation_supports_sessions(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
...
async def test_execute_hub_assistant_mcp_operation_supports_session_latest_messages(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    thread = await create_conversation_thread(
    ...
    result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
...
async def test_execute_hub_assistant_mcp_operation_supports_follow_up_state(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    user_id = user.id
    ...
    set_result = await execute_hub_assistant_mcp_operation(
        user_id=user_id,
        operation_id="hub_assistant.followups.set_sessions",
        arguments={"conversation_ids": [str(target_thread.id)]},
        web_agent_conversation_id=str(hub_assistant_thread.id),
        db=async_db_session,
    )
    get_result = await execute_hub_assistant_mcp_operation(
        user_id=user_id,
    ...
    assert set_result["result"]["hub_assistant_conversation_id"] == str(
        hub_assistant_thread.id
    )
    ...
    assert get_result["result"]["hub_assistant_conversation_id"] == str(
        hub_assistant_thread.id
    )
...
async def test_execute_hub_assistant_mcp_operation_supports_agents(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    agent = await create_a2a_agent(
    ...
    update_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
...
async def test_execute_hub_assistant_mcp_operation_supports_agent_health_checks(
    async_db_session,
    ...
    single_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
        operation_id="hub_assistant.agents.check_health",
        arguments={"agent_id": str(agent.id), "force": True},
        db=async_db_session,
    )
    all_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
...
async def test_execute_hub_assistant_mcp_operation_supports_agent_create_delete(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)

    create_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
    ...
    delete_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
...
async def test_execute_hub_assistant_mcp_operation_supports_job_create_update_delete(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    agent = await create_a2a_agent(
...
async def test_execute_hub_assistant_mcp_operation_rejects_noncanonical_job_conversation_policy(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    agent = await create_a2a_agent(
...
async def test_execute_hub_assistant_mcp_operation_supports_session_writes(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    thread = await create_conversation_thread(
    ...
    archive_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
        operation_id="hub_assistant.sessions.archive",
        arguments={"conversation_id": str(thread.id)},
        db=async_db_session,
    )
    unarchive_result = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
...
async def test_execute_hub_assistant_mcp_operation_supports_session_send_message(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
    ...
    monkeypatch.setattr(
        delegated_conversation_service_module.hub_assistant_delegated_conversation_service,
...
async def test_execute_hub_assistant_mcp_operation_supports_agent_start_sessions(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
...
async def test_execute_hub_assistant_mcp_operation_persists_delegated_session_send(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
    ...
    assert automation_message.message_metadata["delegated_by"] == ("hub_assistant")
    ...
    follow_up_state = await execute_hub_assistant_mcp_operation(
        user_id=user.id,
        operation_id="hub_assistant.followups.get",
        arguments={},
        web_agent_conversation_id=str(hub_assistant_thread.id),
    ...
    work_items = await hub_assistant_task_service.claim_due_tasks(
        db=async_db_session,
        batch_size=10,
    )
    assert len(work_items) == 1
    assert work_items[0].task_kind == HubAssistantTask.KIND_FOLLOW_UP_WATCH
    request = work_items[0].request
    assert request.hub_assistant_conversation_id == str(hub_assistant_thread.id)
    ...
    hub_assistant_messages = list(
        (
            await async_db_session.scalars(
                select(AgentMessage).where(
                    AgentMessage.conversation_id == hub_assistant_thread.id,
                    AgentMessage.sender == "agent",
                )
            )
        ).all()
    ...
    handoff_message = next(
        (
            message
            for message in hub_assistant_messages
...
async def test_execute_hub_assistant_mcp_operation_persists_delegated_agent_start(
    async_db_session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user = await create_user(async_db_session)
...
async def test_hub_assistant_mcp_http_app_requires_bearer_auth(
    async_db_session,
) -> None:
    await create_user(async_db_session)
    mcp_app = build_hub_assistant_mcp_http_app(
    ...
    app.mount(HUB_ASSISTANT_MCP_READONLY_MOUNT_PATH, mcp_app)
    ...
    async with AsyncClient(
        ...
        missing = await client.get(f"{HUB_ASSISTANT_MCP_READONLY_MOUNT_PATH}/")
        invalid = await client.get(
            f"{HUB_ASSISTANT_MCP_READONLY_MOUNT_PATH}/",
...
async def test_hub_assistant_mcp_auth_middleware_accepts_valid_bearer_auth(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    token = create_hub_assistant_access_token(
        user.id,
        allowed_operations=sorted(HUB_ASSISTANT_MCP_READONLY_OPERATION_IDS),
        delegated_by="test",
        conversation_id="hub_assistant-conv-1",
    )
    app = FastAPI()
    app.add_middleware(
        HubAssistantMcpAuthMiddleware,
        default_allowed_operation_ids=HUB_ASSISTANT_MCP_READONLY_OPERATION_IDS,
    ...
    assert response.json()["conversation_id"] == "hub_assistant-conv-1"
    assert response.json()["allowed_operation_ids"] == sorted(
        HUB_ASSISTANT_MCP_READONLY_OPERATION_IDS
    )


async def test_execute_hub_assistant_mcp_operation_rejects_unapproved_operation(
    async_db_session,
) -> None:
    user = await create_user(async_db_session)
    agent = await create_a2a_agent(
    ...
    result = await execute_hub_assistant_mcp_operation(
        ...
        allowed_operation_ids=HUB_ASSISTANT_MCP_READONLY_OPERATION_IDS,
        db=async_db_session,
    )
    ...
    assert result == {
        "ok": False,
        "error": (
            "Operation `hub_assistant.agents.update_config` is not authorized for this MCP session."
        ),
    }
