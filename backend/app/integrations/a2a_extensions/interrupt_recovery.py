...
def resolve_interrupt_recovery(
    ...
    list_method = contract_utils.normalize_method_name(
        methods.get("list"),
        field="methods.list",
    )
    list_permissions_method = contract_utils.normalize_method_name(
        methods.get("list_permissions"),
        field="methods.list_permissions",
    )
    list_questions_method = contract_utils.normalize_method_name(
    ...
    recovery_scope = contract_utils.as_dict(params.get("recovery_scope"))

    raw_recovery_data_source = recovery_scope.get("data_source")
    recovery_data_source = (
        contract_utils.require_str(
            raw_recovery_data_source, field="recovery_scope.data_source"
        )
