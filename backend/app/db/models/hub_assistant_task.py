"""Durable background task owned by the Hub Assistant."""
...
class HubAssistantTask(Base, TimestampMixin, UserOwnedMixin):
    """Persisted background task for one Hub Assistant workflow."""

    __tablename__ = "hub_assistant_tasks"
    __table_args__ = (
        UniqueConstraint(
            "dedupe_key",
            name="uq_hub_assistant_tasks_dedupe_key",
        ),
        Index(
    ...
    hub_assistant_conversation_id = Column(
        UUID(as_uuid=True),
        ForeignKey(f"{SCHEMA_NAME}.conversation_threads.id", ondelete="CASCADE"),
        nullable=False,
        comment="Hub Assistant conversation that owns this task.",
    )
    task_kind = Column(
        String(64),
        nullable=False,
