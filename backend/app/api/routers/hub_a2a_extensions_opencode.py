...
router = StrictAPIRouter(prefix="/a2a/agents", tags=["a2a-extensions"])
...
async def opencode_list_sessions(
    *,
    ...
    logger.info(
        "Hub OpenCode sessions list requested",
        extra={
...
async def opencode_list_sessions_post(
    *,
    ...
    logger.info(
        "Hub OpenCode sessions list requested (POST)",
        extra={
            "user_id": str(current_user.id),
            "agent_id": str(agent_id),
            "agent_url": redact_url_for_logging(runtime.resolved.url),
...
async def opencode_list_session_messages(
    *,
    ...
    query_obj = _parse_query_param(query)
    logger.info(
        "Hub OpenCode session messages requested",
        extra={
            "user_id": str(current_user.id),
            "agent_id": str(agent_id),
            "agent_url": redact_url_for_logging(runtime.resolved.url),
...
async def opencode_list_session_messages_post(
    *,
    ...
    logger.info(
        "Hub OpenCode session messages requested (POST)",
        extra={
            "user_id": str(current_user.id),
            "agent_id": str(agent_id),
            "agent_url": redact_url_for_logging(runtime.resolved.url),
