"""Shared operation-oriented toolkit for Hub Assistant entry points."""
...
from app.features.hub_assistant_shared.delegated_conversation_service import (
...
class HubAssistantToolInputError(ValueError):
    """Raised when a Hub Assistant tool invocation has invalid inputs."""


@dataclass(frozen=True)
class HubAssistantToolExecutionResult:
    """One structured Hub Assistant tool execution result."""

    operation_id: str
    payload: dict[str, Any]


class HubAssistantToolkit:
    """Operation-oriented toolkit shared by CLI and future Hub Assistants."""
    ...
    async def execute(
        ...
        *,
        ...
        elif operation.operation_id == HUB_ASSISTANT_JOBS_CREATE.operation_id:
            payload = await self._create_job(args)
        elif operation.operation_id == HUB_ASSISTANT_JOBS_PAUSE.operation_id:
            payload = await self._pause_job(args)
        elif operation.operation_id == HUB_ASSISTANT_JOBS_RESUME.operation_id:
            payload = await self._resume_job(args)
        elif operation.operation_id == HUB_ASSISTANT_JOBS_UPDATE.operation_id:
            payload = await self._update_job(args)
        elif operation.operation_id == HUB_ASSISTANT_JOBS_UPDATE_PROMPT.operation_id:
            payload = await self._update_job_prompt(args)
        elif operation.operation_id == HUB_ASSISTANT_JOBS_UPDATE_SCHEDULE.operation_id:
            payload = await self._update_job_schedule(args)
        elif operation.operation_id == HUB_ASSISTANT_JOBS_DELETE.operation_id:
            payload = await self._delete_job(args)
        elif operation.operation_id == HUB_ASSISTANT_SESSIONS_LIST.operation_id:
        ...
        elif operation.operation_id == HUB_ASSISTANT_SESSIONS_UPDATE.operation_id:
            payload = await self._update_session(args)
        elif operation.operation_id == HUB_ASSISTANT_SESSIONS_ARCHIVE.operation_id:
            payload = await self._archive_session(args)
        elif operation.operation_id == HUB_ASSISTANT_SESSIONS_UNARCHIVE.operation_id:
            payload = await self._unarchive_session(args)
        elif operation.operation_id == HUB_ASSISTANT_SESSIONS_SEND_MESSAGE.operation_id:
            payload = await self._send_session_message(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_LIST.operation_id:
            payload = await self._list_agents(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_GET.operation_id:
            payload = await self._get_agent(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_CHECK_HEALTH.operation_id:
            payload = await self._check_agent_health(args)
        elif (
            operation.operation_id == HUB_ASSISTANT_AGENTS_CHECK_HEALTH_ALL.operation_id
        ):
            payload = await self._check_all_agents_health(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_CREATE.operation_id:
            payload = await self._create_agent(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_UPDATE_CONFIG.operation_id:
            payload = await self._update_agent_config(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_DELETE.operation_id:
            payload = await self._delete_agent(args)
        elif operation.operation_id == HUB_ASSISTANT_AGENTS_START_SESSIONS.operation_id:
            payload = await self._start_agent_sessions(args)
        else:  # pragma: no cover - defensive guard
            raise HubAssistantToolInputError(
                f"Operation `{operation_id}` is not implemented by the toolkit."
            )
        return HubAssistantToolExecutionResult(
    ...
    async def _get_job(self, args: dict[str, Any]) -> dict[str, Any]:
        task = await hub_assistant_jobs_service.get_job(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
    ...
    async def _create_job(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        task = await hub_assistant_jobs_service.create_job(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
    ...
    async def _pause_job(self, args: dict[str, Any]) -> dict[str, Any]:
        task = await hub_assistant_jobs_service.pause_job(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            task_id=self._as_uuid(args.get("task_id"), field_name="task_id"),
    ...
    async def _resume_job(self, args: dict[str, Any]) -> dict[str, Any]:
        task = await hub_assistant_jobs_service.resume_job(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            task_id=self._as_uuid(args.get("task_id"), field_name="task_id"),
    ...
    async def _update_job(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        task = await hub_assistant_jobs_service.update_job(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            task_id=self._as_uuid(args.get("task_id"), field_name="task_id"),
    ...
    async def _update_job_prompt(self, args: dict[str, Any]) -> dict[str, Any]:
        prompt = self._as_str(args.get("prompt"), field_name="prompt")
        task = await hub_assistant_jobs_service.update_prompt(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            task_id=self._as_uuid(args.get("task_id"), field_name="task_id"),
    ...
    async def _update_job_schedule(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        task = await hub_assistant_jobs_service.update_schedule(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            task_id=self._as_uuid(args.get("task_id"), field_name="task_id"),
    ...
    async def _list_sessions(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        items, extra, _db_mutated = await hub_assistant_sessions_service.list_sessions(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            page=page,
    ...
    async def _get_session(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        session_item = await hub_assistant_sessions_service.get_session(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
    ...
    async def _get_latest_session_messages(
        ...
        payload = await hub_assistant_sessions_service.get_latest_messages(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
    ...
    async def _update_session(self, args: dict[str, Any]) -> dict[str, Any]:
        session_item = await hub_assistant_sessions_service.update_session(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            conversation_id=self._as_str(
    ...
    async def _archive_session(self, args: dict[str, Any]) -> dict[str, Any]:
        session_item = await hub_assistant_sessions_service.archive_session(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            conversation_id=self._as_str(
    ...
    async def _unarchive_session(self, args: dict[str, Any]) -> dict[str, Any]:
        session_item = await hub_assistant_sessions_service.unarchive_session(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            conversation_id=self._as_str(
    ...
    async def _send_session_message(self, args: dict[str, Any]) -> dict[str, Any]:
        return await hub_assistant_delegated_conversation_service.send_messages_to_sessions(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
    ...
    async def _get_agent(self, args: dict[str, Any]) -> dict[str, Any]:
        record = await hub_assistant_agents_service.get_agent(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            agent_id=self._as_uuid(args.get("agent_id"), field_name="agent_id"),
    ...
    async def _check_agent_health(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        summary, items = await hub_assistant_agents_service.check_agent_health(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            agent_id=self._as_uuid(args.get("agent_id"), field_name="agent_id"),
    ...
    async def _check_all_agents_health(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        summary, items = await hub_assistant_agents_service.check_all_agents_health(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            force=force,
    ...
    async def _create_agent(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        record = await hub_assistant_agents_service.create_agent(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            name=self._as_str(args.get("name"), field_name="name"),
    ...
    async def _update_agent_config(self, args: dict[str, Any]) -> dict[str, Any]:
        record = await hub_assistant_agents_service.update_config(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
            agent_id=self._as_uuid(args.get("agent_id"), field_name="agent_id"),
    ...
    async def _start_agent_sessions(self, args: dict[str, Any]) -> dict[str, Any]:
        return await hub_assistant_delegated_conversation_service.start_sessions_for_agents(
            db=self.db,
            gateway=self.gateway,
            current_user=self.current_user,
    ...
    def _as_int(value: Any, *, field_name: str, minimum: int) -> int:
        ...
        if resolved < minimum:
            raise HubAssistantToolInputError(f"`{field_name}` must be >= {minimum}.")
        return resolved
    ...
    def _as_optional_str(value: Any) -> str | None:
        ...
        if not isinstance(value, str):
            raise HubAssistantToolInputError("Expected a string value.")
        return value
    ...
    def _as_optional_bool(value: Any, *, field_name: str) -> bool | None:
        ...
        if isinstance(value, bool):
            return value
        raise HubAssistantToolInputError(f"`{field_name}` must be a boolean.")
    ...
    def _as_optional_dict(
        ...
        *,
        ...
        if not isinstance(value, dict):
            raise HubAssistantToolInputError(f"`{field_name}` must be an object.")
    ...
    def _as_optional_str_list(
        ...
        *,
        ...
        if not isinstance(value, list):
            raise HubAssistantToolInputError(f"`{field_name}` must be an array.")
        normalized: list[str] = []
        for item in value:
    ...
    def _as_session_source(value: str | None) -> SessionSource | None:
        ...
        raise HubAssistantToolInputError("`source` must be one of: manual, scheduled.")
