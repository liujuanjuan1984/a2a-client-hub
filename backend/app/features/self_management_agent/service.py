...
_DEFAULT_SYSTEM_PROMPT = (
    ...
    "In this built-in conversation, the host automatically adds accepted "
    "delegated target conversations to durable follow-up tracking. Do not ask "
    "the user whether you should keep following up merely to enable that "
    "default behavior. Use `self.followups.set_sessions` only when you need to "
    "override the current tracked target conversation ids, such as narrowing, "
    "extending, replacing, or stopping tracking by passing an empty list. The "
    "host may later resume you after new persisted target-session text results "
    "arrive. During user-visible or host-resumed follow-up work, use "
    "`self.followups.get` to inspect the current durable tracking state and use "
    "the session read tools to consume persisted target-session text results. "
    "Never wait on downstream live transport or invent downstream outcomes."
)
_FOLLOW_UP_RESUME_MESSAGE_TEMPLATE = (
    ...
    "override the current tracked target conversation ids for future automatic "
    "follow-up, call `self.followups.set_sessions` with the exact next target "
    "conversation ids. Pass an empty list when tracking should stop. Do not "
    "wait on downstream live transport."
)
