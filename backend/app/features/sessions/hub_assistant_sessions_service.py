"""Shared Hub Assistant session services for self-owned conversation threads."""
...
class HubAssistantSessionsService:
    """Shared session operations for REST, CLI, and Hub Assistant entry points."""
    ...
    async def list_sessions(
        ...
        *,
        ...
        result = await gateway.execute(
            operation=HUB_ASSISTANT_SESSIONS_LIST,
    ...
    async def get_session(
        ...
        *,
        ...
        result = await gateway.execute(
            operation=HUB_ASSISTANT_SESSIONS_GET,
    ...
    async def get_latest_messages(
        ...
        *,
        ...
        for conversation_id in self._dedupe_ids(conversation_ids):
            gateway.authorize(
                operation=HUB_ASSISTANT_SESSIONS_GET_LATEST_MESSAGES,
                resource_id=conversation_id,
            )
            try:
                resolved_conversation_id = UUID(conversation_id)
    ...
    async def update_session(
        ...
        *,
        ...
        resolved_conversation_id = UUID(conversation_id)
        result = await gateway.execute(
            operation=HUB_ASSISTANT_SESSIONS_UPDATE,
            resource_id=conversation_id,
    ...
    async def archive_session(
        ...
        *,
        ...
        resolved_conversation_id = UUID(conversation_id)
        result = await gateway.execute(
            operation=HUB_ASSISTANT_SESSIONS_ARCHIVE,
            resource_id=conversation_id,
    ...
    async def unarchive_session(
        ...
        *,
        ...
        resolved_conversation_id = UUID(conversation_id)
        result = await gateway.execute(
            operation=HUB_ASSISTANT_SESSIONS_UNARCHIVE,
            resource_id=conversation_id,
            handler=lambda: self._set_thread_status(
                db=db,
                user_id=self._user_id(current_user),
...
hub_assistant_sessions_service = HubAssistantSessionsService()
