"""Durable background task substrate for the built-in self-management agent."""
...
SelfManagementAgentTaskKind = Literal[
    "follow_up_watch",
...
class SelfManagementFollowUpTaskRequest:
...
@dataclass(frozen=True)
class SelfManagementAgentTaskWorkItem:
    """One claimed durable task ready for execution."""

    task_id: UUID
    user_id: UUID
    built_in_conversation_id: str
    task_kind: SelfManagementAgentTaskKind
    request: (
...
@dataclass(frozen=True)
class SelfManagementFollowUpTaskCompletion:
...
class SelfManagementAgentTaskService:
    """Persist, inspect, and dispatch durable built-in agent tasks."""
    ...
    async def enqueue_permission_reply_continuation(
        ...
        *,
        ...
        task = SelfManagementAgentTask(
            user_id=request.current_user_id,
            built_in_conversation_id=UUID(request.built_in_conversation_id),
            task_kind=SelfManagementAgentTask.KIND_PERMISSION_REPLY_CONTINUATION,
            dedupe_key=dedupe_key,
            task_payload={
    ...
    async def enqueue_delegated_invoke(
        ...
        *,
        ...
        task = SelfManagementAgentTask(
            user_id=request.current_user_id,
            built_in_conversation_id=UUID(request.built_in_conversation_id),
            task_kind=SelfManagementAgentTask.KIND_DELEGATED_INVOKE,
            task_payload={
    ...
    async def recover_stale_running_tasks(
        ...
        rows = list(
            (
                await db.scalars(
                    select(SelfManagementAgentTask)
                    .where(
                        SelfManagementAgentTask.status
                        == SelfManagementAgentTask.STATUS_RUNNING
                    )
        ...
        for task in rows:
            ...
            if started_at is None:
                setattr(task, "status", SelfManagementAgentTask.STATUS_WAITING)
                recovered += 1
                continue
    ...
    async def claim_due_tasks(
        ...
        *,
        db: AsyncSession,
        batch_size: int,
    ) -> list[SelfManagementAgentTaskWorkItem]:
        now = utc_now()
        claimed: list[SelfManagementAgentTaskWorkItem] = []
        mutated = False
        ...
        dispatch_rows = await self._get_waiting_rows(
            ...
            task_kinds=(
                ...
                SelfManagementAgentTask.KIND_DELEGATED_INVOKE,
            ),
            batch_size=batch_size,
        )
        for task in dispatch_rows:
            setattr(task, "status", SelfManagementAgentTask.STATUS_RUNNING)
        ...
        if remaining:
            follow_up_rows = await self._get_waiting_rows(
                db=db,
                task_kinds=(SelfManagementAgentTask.KIND_FOLLOW_UP_WATCH,),
    ...
    async def complete_task(
        ...
        *,
        db: AsyncSession,
        task_id: UUID,
        follow_up_completion: SelfManagementFollowUpTaskCompletion | None = None,
    ) -> None:
        ...
        if task.task_kind == SelfManagementAgentTask.KIND_FOLLOW_UP_WATCH:
            ...
            if task.status != SelfManagementAgentTask.STATUS_RUNNING:
                return
            ...
            setattr(
                ...
                "status",
                (
                    SelfManagementAgentTask.STATUS_WAITING
                    if tracked_conversation_ids
                    else SelfManagementAgentTask.STATUS_COMPLETED
                ),
            )
        ...
        setattr(task, "status", SelfManagementAgentTask.STATUS_COMPLETED)
        setattr(task, "last_run_finished_at", utc_now())
        setattr(task, "last_run_error", None)
        await commit_safely(db)
    ...
    async def fail_task(
        ...
        *,
        ...
        if task is None:
            return
        setattr(task, "status", SelfManagementAgentTask.STATUS_FAILED)
        setattr(task, "last_run_finished_at", utc_now())
        ...
        *,
        ...
        if task is None:
            task = SelfManagementAgentTask(
                ...
                task_kind=SelfManagementAgentTask.KIND_FOLLOW_UP_WATCH,
                task_payload={
    ...
    async def _claim_follow_up_task(
        ...
        *,
        ...
        task: SelfManagementAgentTask,
        now: Any,
    ) -> tuple[SelfManagementAgentTaskWorkItem | None, bool]:
        ...
        return (
            SelfManagementAgentTaskWorkItem(
                ...
                task_kind=cast(
                    SelfManagementAgentTaskKind,
                    SelfManagementAgentTask.KIND_FOLLOW_UP_WATCH,
                ),
            ...
            (
                await db.scalars(
                    ...
                    .where(
                        SelfManagementAgentTask.status
                        == SelfManagementAgentTask.STATUS_WAITING,
                        SelfManagementAgentTask.task_kind.in_(task_kinds),
                    )
                    .order_by(SelfManagementAgentTask.updated_at.asc())
                    .limit(batch_size)
                    .with_for_update(skip_locked=True)
                )
            ).all()
    ...
    ) -> SelfManagementAgentTask | None:
        statement = select(SelfManagementAgentTask).where(
            SelfManagementAgentTask.id == task_id
        )
        if for_update:
            statement = statement.with_for_update()
        return cast(SelfManagementAgentTask | None, await db.scalar(statement))
    ...
    async def _get_task_by_dedupe_key(
        ...
        *,
        ...
        statement = select(SelfManagementAgentTask).where(
            SelfManagementAgentTask.dedupe_key == dedupe_key
        )
        if for_update:
            statement = statement.with_for_update()
        return cast(SelfManagementAgentTask | None, await db.scalar(statement))
    ...
    async def _get_follow_up_task_by_conversation(
        ...
        *,
        ...
        statement = select(SelfManagementAgentTask).where(
            SelfManagementAgentTask.user_id == user_id,
            SelfManagementAgentTask.task_kind
            == SelfManagementAgentTask.KIND_FOLLOW_UP_WATCH,
            SelfManagementAgentTask.built_in_conversation_id
            == UUID(built_in_conversation_id),
        )
        if for_update:
            statement = statement.with_for_update()
        return cast(SelfManagementAgentTask | None, await db.scalar(statement.limit(1)))
    ...
    def _deserialize_task(
        ...
        task_kind = cast(SelfManagementAgentTaskKind, task.task_kind)
        request: PermissionReplyContinuationTaskRequest | DelegatedInvokeTaskRequest
        if task_kind == SelfManagementAgentTask.KIND_PERMISSION_REPLY_CONTINUATION:
            request = PermissionReplyContinuationTaskRequest(
        ...
        elif task_kind == SelfManagementAgentTask.KIND_DELEGATED_INVOKE:
            request = DelegatedInvokeTaskRequest(
                current_user_id=cast(UUID, task.user_id),
                built_in_conversation_id=str(cast(UUID, task.built_in_conversation_id)),
            ...
            raise ValueError(f"Unsupported self-management task kind: {task_kind}")
        return SelfManagementAgentTaskWorkItem(
            task_id=cast(UUID, task.id),
            user_id=cast(UUID, task.user_id),
            built_in_conversation_id=str(cast(UUID, task.built_in_conversation_id)),
            task_kind=task_kind,
    ...
    def _follow_up_tracked_conversation_ids(
        self,
        task: SelfManagementAgentTask | None,
    ) -> list[str]:
        if task is None:
            return []
        payload = cast(dict[str, Any], task.task_payload or {})
...
self_management_agent_task_service = SelfManagementAgentTaskService()
