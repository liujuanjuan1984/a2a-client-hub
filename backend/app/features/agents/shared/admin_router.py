...
from app.features.agents.shared.schemas import (
    SharedAgentAdminCreate,
    SharedAgentAdminListResponse,
    SharedAgentAdminResponse,
    SharedAgentAdminUpdate,
    SharedAgentAllowlistAddRequest,
    SharedAgentAllowlistEntryResponse,
    SharedAgentAllowlistListResponse,
    SharedAgentAllowlistReplaceRequest,
    SharedAgentListMeta,
    SharedAgentPagination,
)
from app.features.agents.shared.service import (
    SharedAgentAllowlistConflictError,
    SharedAgentNotFoundError,
    SharedAgentRecord,
    SharedAgentUserNotFoundError,
...
def _build_admin_response(record: SharedAgentRecord) -> SharedAgentAdminResponse:
    ...
    return SharedAgentAdminResponse.model_validate(payload)


@router.get("", response_model=SharedAgentAdminListResponse)
async def list_shared_agents_admin(
    *,
    ...
    items, total = await shared_agent_service.list_agents_admin(
        db, page=page, size=size
    )
    pages = (total + size - 1) // size if size else 0
    return SharedAgentAdminListResponse(
...
@router.post(
    "",
    response_model=SharedAgentAdminResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_shared_agent_admin(
    *,
    payload: SharedAgentAdminCreate,
    response: Response,
    db: AsyncSession = Depends(get_async_db),
    gateway: HubOperationGateway = Depends(get_current_hub_admin_operation_gateway),
) -> SharedAgentAdminResponse:
    ...
    try:
        record = await shared_agent_service.create_agent_admin(
            db,
...
@router.get("/{agent_id}", response_model=SharedAgentAdminResponse)
async def get_shared_agent_admin(
    *,
    agent_id: UUID,
    db: AsyncSession = Depends(get_async_db),
    gateway: HubOperationGateway = Depends(get_current_hub_admin_operation_gateway),
    ...
    try:
        record = await shared_agent_service.get_agent_admin(db, agent_id=agent_id)
    except SharedAgentNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    return _build_admin_response(record)


@router.put("/{agent_id}", response_model=SharedAgentAdminResponse)
async def update_shared_agent_admin(
    *,
    agent_id: UUID,
    payload: SharedAgentAdminUpdate,
    response: Response,
    db: AsyncSession = Depends(get_async_db),
    gateway: HubOperationGateway = Depends(get_current_hub_admin_operation_gateway),
) -> SharedAgentAdminResponse:
    ...
    try:
        record = await shared_agent_service.update_agent_admin(
            db,
            admin_user_id=current_admin_id,
            agent_id=agent_id,
            name=payload.name,
...
async def delete_shared_agent_admin(
    *,
    agent_id: UUID,
    db: AsyncSession = Depends(get_async_db),
    gateway: HubOperationGateway = Depends(get_current_hub_admin_operation_gateway),
    ...
    current_admin_id = gateway.actor.acting_user_id
    try:
        await shared_agent_service.delete_agent_admin(
...
@router.get(
    "/{agent_id}/allowlist",
    response_model=SharedAgentAllowlistListResponse,
...
async def list_hub_agent_allowlist_admin(
    *,
    ...
    try:
        records = await shared_agent_service.list_allowlist_entries_admin(
            db, agent_id=agent_id
        )
    except SharedAgentNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    return SharedAgentAllowlistListResponse(
        items=[
            SharedAgentAllowlistEntryResponse(
                id=item.id,
...
@router.post(
    "/{agent_id}/allowlist",
    response_model=SharedAgentAllowlistEntryResponse,
    status_code=status.HTTP_201_CREATED,
)
async def add_hub_agent_allowlist_admin(
    *,
    agent_id: UUID,
    payload: SharedAgentAllowlistAddRequest,
    response: Response,
    db: AsyncSession = Depends(get_async_db),
    gateway: HubOperationGateway = Depends(get_current_hub_admin_operation_gateway),
) -> SharedAgentAllowlistEntryResponse:
    ...
    try:
        record = await shared_agent_service.add_allowlist_entry_admin(
            db,
            admin_user_id=current_admin_id,
            agent_id=agent_id,
        ...
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except SharedAgentAllowlistConflictError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except SharedAgentUserNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except SharedAgentValidationError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return SharedAgentAllowlistEntryResponse(
        id=record.id,
...
async def replace_hub_agent_allowlist_admin(
    *,
    agent_id: UUID,
    payload: SharedAgentAllowlistReplaceRequest,
    response: Response,
    db: AsyncSession = Depends(get_async_db),
    gateway: HubOperationGateway = Depends(get_current_hub_admin_operation_gateway),
) -> SharedAgentAllowlistListResponse:
    ...
    try:
        records = await shared_agent_service.replace_allowlist_entries_admin(
            db,
            admin_user_id=current_admin_id,
            agent_id=agent_id,
            entries=[
...
async def remove_hub_agent_allowlist_admin(
    *,
    ...
    try:
        await shared_agent_service.remove_allowlist_entry_admin(
