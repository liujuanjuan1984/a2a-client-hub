...
- Upstream A2A task state is fetched on demand via `GET /me/conversations/{conversation_id}/upstream-tasks/{task_id}`. Recovery/detail flows should call that endpoint only when diagnostics are needed; the app intentionally does not poll upstream task state globally.
